...
Программа работает в фоне и стартует вместе с Windows — между встречами она может
простаивать часами, а модель Whisper всё это время держит несколько гигабайт VRAM
занятыми, даже если стадией `analyze` вы вообще не пользуетесь. По умолчанию через
`IDLE_UNLOAD_SECONDS=300` (5 минут) простоя без расшифровок и анализов в очереди
все модели выгружаются из видеокарты. Следующая расшифровка после выгрузки начнётся
с повторной загрузки модели — это единицы секунд, но на каждый файл, поэтому не
ставьте значение слишком маленьким. `0` — не выгружать вовсе. Меняется в `.env`.
