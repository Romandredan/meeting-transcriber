...
**Главная страница** — добавление записей, настройки распознавания и очередь (с фильтром по типу анализа и переименованием встреч прямо в списке):
...
- **Темы оформления.** Пять пресетов на выбор в шапке: «Матрица» (глубокий синий с приглушённым дождём кода фоном — по умолчанию), «Чертёж» (тёмная), «День» (светлая), «Уголь» (почти чёрная) и «Терминал» (зелёный акцент). Выбор запоминается в браузере.
...
- **Названия встреч.** Автоматические имена файлов вида «Видеовстреча 2026-03-31_140328.webm» через месяц ни о чём не говорят — встречу можно переименовать карандашом в списке. Название показывается в списке, находится поиском и подставляется в имя файла при скачивании; сам файл на диске не переименовывается.
- **Фильтр по типу анализа.** Дропдаун «Тип анализа» над списком оставляет только встречи, у которых есть анализ выбранного шаблона (со счётчиками: «Протокол встречи (7)»). Комбинируется с фильтрами статуса и поиском по названию.
...
| `GET /api/jobs` | список встреч (`?analysis=<метка>` — только встречи, у которых есть анализ этого типа) |
| `GET /api/jobs/{id}` | одна встреча |
| `PATCH /api/jobs/{id}/title` | переименовать встречу (отображаемое название; пустая строка — сброс к имени файла) |
...
| `GET /api/analysis_labels` | метки анализов со счётчиками встреч — варианты фильтра по типу анализа¹ |
