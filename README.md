...
- **Темы оформления.** Пять пресетов на выбор в шапке: «Чертёж» (тёмная, по умолчанию), «День» (светлая), «Уголь» (почти чёрная), «Терминал» (зелёный акцент) и «Матрица» (глубокий синий с дождём кода фоном). Выбор запоминается в браузере.
